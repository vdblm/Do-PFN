...
from priors.playground_scm.MakeStructuralEquationsConstantNoise_oppo import MakeStructuralEquationsConstantNoise, MakeMakeStructuralEquationsConstantNoiseSquareNonlinearity, MakeMakeStructuralEquationsConstantNoiseTanHNonlinearity
from priors.playground_scm.MakeStructuralEquationsConstantNoise_oppo import make_additive_noise_sampling, make_exo_dist_samples, make_distribution_mixture_loc_scale_one_dist_per_feature
    ...
    class DoSCM(torch.nn.Module):
        ...
        def forward(self):  
            ...
            if self.inference_cov == 'counterfactual':
                ...
                if 'X' in self.scm.t_key:  # if enogenous variable, change functional mechanism
                    t_int = 1 - endo_obs[self.scm.t_key]
                    self.scm.do_interventions([(self.scm.t_key, (lambda: t_int, {}))])
                else:  # if exogenous variable, change the value of the variable directly
                    t_int = 1 - exo_obs[self.scm.t_key]
