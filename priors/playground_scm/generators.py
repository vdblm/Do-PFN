...
class SCMGenerator:
    ...
    def __init__(self, all_functions: Dict[str, Callable], seed: int, samples_shape: tuple, noise_std: float, noise_dist: str, nonlins: str, max_hidden_layers: int):
        """
    ...
    def create_scm_from_graph(self, graph: nx.DiGraph, possible_functions: List[str], exo_distribution: Callable,
        ...
        for n in graph.nodes:
            parents = list(graph.predecessors(n))
            graph.nodes[n]['type'] = "endo" if len(parents) > 0 else "exo"
            if graph.nodes[n]['type'] == 'endo':
                current_function = random.choice(possible_functions)
                scm.add_endogenous_var(n, self.all_functions[current_function](parents=parents, samples_shape=self.samples_shape, noise_std=self.noise_std, noise_dist=self.noise_dist, nonlins=self.nonlins, max_hidden_layers=self.max_hidden_layers), {p: p for p in parents})
            else:
    ...
    def create_graph_from_nodes(self, num_nodes, p) -> nx.DiGraph:
        ...
        # create random permutation of nodes (topological order)
